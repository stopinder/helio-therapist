...
test.describe('Clinical Record approval and amendment flow', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await page.route('**/rest/v1/therapist_reminders*', route =>
            route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify([])
        ...
        await page.route(
            '**/rest/v1/rpc/save_session_draft',
            async route => {
        ...
        await page.route(
            '**/rest/v1/clinical_record_amendments*',
            route =>
                route.fulfill({
                    status: 200,
                    contentType: 'application/json',
        ...
        await page.route(
            '**/rest/v1/rpc/approve_clinical_record_amendment',
            async route => {
                const body = route.request().postDataJSON();
        ...
        await page.route('**/api/ai/transcribe', async route => {
            ...
            expect(body.audio).toMatch(
                /^data:audio\/webm;base64,/
            );

            const text =
                transcribeCalls === 1
                    ? 'Factual correction dictated.'
                    : 'Client reported four hours of sleep, not five, dictated.';
        ...
        await page.route('**/api/client-timeline*', route =>
            route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({
                            ...
                            ? [
                                {
                                    ...
                                    summary:
                                        'Clinical record approved for this session.'
                                }
                            ]
                            : []
                })
    ...
    test(
        'approves a draft, shows completion handoff, approves an amendment, then reopens the immutable record',
        async ({ page }) => {
            await openWorkspace(page);

            await page
                .getByRole('tab', { name: 'Clinical Record' })
                .click();

            await page
                .getByRole('button', {
                    name: 'Prepare empty Clinical Record draft'
                })
                .click();

            await page
                .getByLabel('Presenting concerns')
                .fill(
                    'Client described increased work stress.'
                );

            await page
                .getByLabel('Plan for next session')
                .fill(
                    'Review coping strategies and sleep routine.'
                );
            ...
            await approvalDialog
                .getByLabel(
                    'I confirm that this summary accurately represents the session.'
                )
                .check();
            ...
            await approvalDialog
                .getByRole('button', {
                    name:
                        'Approve and Create Clinical Record'
                })
                .click();
            ...
            await expect(
                handoff.getByText(
                    'Clinical record approved and locked.'
                )
            ).toBeVisible();

            await expect(
                handoff.getByRole('link', {
            ...
            await expect(
                completedRecord.getByText(
                    'This approved record is read-only. Corrections must be added through an amendment.'
                )
            ).toBeVisible();
            ...
            await expect(
                completedRecord.locator('textarea')
            ).toHaveCount(0);
            ...
            const editor =
                page.getByTestId(
                    'clinical-record-amendment-editor'
                );

            const dictateButtons =
            ...
            await editor
                ...
                .fill(
                    'Client reported four hours of sleep, not five.'
                );
            ...
            await expect(
                reopenedRecord.getByText(
                    'Record Amendment 1'
                )
            ).toBeVisible();

            await expect(
                reopenedRecord.locator(
                    'textarea'
                )
            ).toHaveCount(0);
        }
