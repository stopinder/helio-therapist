...
test.describe('Clinical Record approval and amendment flow', () => {
  ...
  let sessionRow; let amendments; let transcribeCalls;
  ...
  test.beforeEach(async ({ page }) => {
    ...
    amendments = []; transcribeCalls = 0;
    await page.addInitScript(() => { const track={stop(){}}; Object.defineProperty(navigator,'mediaDevices',{configurable:true,value:{getUserMedia:async()=>({getTracks:()=>[track]})}}); class MockMediaRecorder{constructor(){this.state='inactive';this.mimeType='audio/webm';this.ondataavailable=null;this.onstop=null}start(){this.state='recording'}stop(){this.state='inactive';this.ondataavailable?.({data:new Blob(['mock-audio'],{type:'audio/webm'})});this.onstop?.()}} window.MediaRecorder=MockMediaRecorder; });
  ...
  test('approves a draft, shows completion handoff, approves an amendment, then reopens the immutable record', async ({ page }) => {
    ...
    const approvalDialog=page.getByRole('dialog',{name:'Create Clinical Record'}); await expect(approvalDialog).toBeVisible(); await approvalDialog.getByLabel('I confirm that this summary accurately represents the session.').check(); await approvalDialog.getByRole('button',{name:'Approve and Create Clinical Record'}).click();
    const handoff=page.getByTestId('session-completion-handoff'); await expect(handoff).toBeVisible(); await expect(handoff.getByText('Clinical record approved and locked.')).toBeVisible(); await expect(handoff.getByRole('link',{name:'Return to client'})).toHaveAttribute('href',`/clients/${clientId}`); await expect(handoff.getByRole('link',{name:'Schedule next appointment'})).toHaveAttribute('href',`/schedule?clientId=${clientId}`);
    const completedRecord=page.getByTestId('completed-clinical-record'); await expect(completedRecord).toBeVisible(); await expect(completedRecord.getByText('This approved record is read-only. Corrections must be added through an amendment.')).toBeVisible(); await expect(completedRecord.getByText('Client described increased work stress.')).toBeVisible(); await expect(completedRecord.locator('textarea')).toHaveCount(0);
    await completedRecord.getByRole('button',{name:'Create Record Amendment'}).click(); const editor=page.getByTestId('clinical-record-amendment-editor'); const dictateButtons=editor.getByRole('button',{name:'Dictate'}); await expect(dictateButtons).toHaveCount(2); await dictateButtons.nth(0).click(); await expect(editor.getByRole('button',{name:'Stop recording'})).toBeVisible(); await editor.getByRole('button',{name:'Stop recording'}).click(); await expect(editor.getByLabel('Amendment Reason')).toHaveValue('Factual correction dictated.'); await editor.getByLabel('Amendment Reason').fill('Factual correction'); await editor.getByRole('button',{name:'Dictate'}).nth(1).click(); await editor.getByRole('button',{name:'Stop recording'}).click(); await expect(editor.getByLabel('Amendment Content')).toHaveValue('Client reported four hours of sleep, not five, dictated.'); await editor.getByLabel('Amendment Content').fill('Client reported four hours of sleep, not five.'); expect(transcribeCalls).toBe(2); await editor.getByLabel('I have reviewed this amendment and confirm it should be appended to the clinical record.').check(); await editor.getByRole('button',{name:'Approve Amendment'}).click(); await expect(editor).toBeHidden(); await expect(completedRecord.getByText('Record Amendment 1')).toBeVisible();
    await handoff.getByRole('link',{name:'Return to client'}).click(); await expect(page).toHaveURL(`/clients/${clientId}`); await page.getByRole('button',{name:'Timeline'}).click(); await page.getByText('Clinical record approved for this session.').click(); await expect(page).toHaveURL(`/clients/${clientId}/sessions/${sessionId}`); await page.getByRole('button',{name:/Clinical Record/}).click(); await expect(page.getByTestId('session-completion-handoff')).toBeVisible(); const reopenedRecord=page.getByTestId('completed-clinical-record'); await expect(reopenedRecord.getByText('Record Amendment 1')).toBeVisible(); await expect(reopenedRecord.locator('textarea')).toHaveCount(0);
  });
});
