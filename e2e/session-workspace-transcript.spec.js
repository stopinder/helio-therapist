  ...
  test.beforeEach(async ({ page }) => {
    await page.route('**/auth/v1/token*', route => route.fulfill({
      ...
      body: JSON.stringify({
        ...
        user: { id: userId, email: 'therapist@example.com', role: 'authenticated', aud: 'authenticated' }
      })
    }));
    await page.route('**/auth/v1/user', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ id: userId, email: 'therapist@example.com' })
    }));
    ...
    await page.route('**/rest/v1/clients*', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify([{ id: clientId, display_name: 'Test Client', archived: false }])
    }));
  ...
  test('renders the exact linked Zoom source without demonstration fallback', async ({ page }) => {
    ...
    await page.route('**/api/zoom/transcripts?*', async route => {
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ transcripts: [{ id: 'transcript-1', sessionRef: sessionId, clientId, text: sourceText }] })
      });
    });
    ...
    await page.route('**/api/zoom/transcripts?*', route => {
      ...
      if (requests === 1) {
        return route.fulfill({
          status: 500,
          contentType: 'application/json',
          body: JSON.stringify({ error: 'Temporary transcript error' })
        });
      }
      return route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ transcripts: [{ id: 'transcript-1', sessionRef: sessionId, clientId, text: 'Recovered source transcript' }] })
      });
    });
