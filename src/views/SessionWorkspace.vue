...
async function loadSession(){loading.value=true;error.value='';try{const {clientId,sessionId}=route.params;const [sessionData,clientData]=await Promise.all([getSession({clientId,sessionId}),getClient({clientId})]);session.value=sessionData;client.value=clientData;now.value=Date.now();loadTranscript()}catch(e){error.value='The clinical workspace could not be loaded.'}finally{loading.value=false}}
...
const elapsedTime=computed(()=>{if(!session.value?.startedAt)return'00:00:00';const seconds=Math.max(0,Math.floor((now.value-new Date(session.value.startedAt).getTime())/1000));const h=Math.floor(seconds/3600),m=Math.floor((seconds%3600)/60),s=seconds%60;return[h,m,s].map(v=>String(v).padStart(2,'0')).join(':')}); const workspaceSession=computed(()=>{if(!session.value)return null;const dateObj=new Date(session.value.startedAt),isValidDate=!isNaN(dateObj.getTime());return{id:session.value.id,clientId:session.value.clientId,status:session.value.status==='in_progress'?'In Progress':(session.value.status==='completed'?'Completed':session.value.status),clientName:client.value?.display_name||client.value?.name||'Unknown Client',date:isValidDate?dateObj.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}):'Pending',time:isValidDate?dateObj.toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',hour12:true}):'',type:'Clinical session',videoProvider:'zoom',isInPerson:false,elapsedTime:elapsedTime.value}});
onMounted(()=>{clockTimer=setInterval(()=>{now.value=Date.now()},1000);loadSession()}); onBeforeUnmount(()=>clearInterval(clockTimer));
</script>
