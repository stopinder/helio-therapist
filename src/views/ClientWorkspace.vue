      ...
      <div class="flex-1 overflow-auto p-inline-lg py-stack-lg">
        <div class="max-w-6xl mx-auto space-y-stack-xl">
          ...
          <section aria-labelledby="recent-sessions-heading" class="bg-surface-elevated border border-border-muted rounded-panel p-inline-lg">
            <header class="flex items-center justify-between mb-stack-md pt-stack-md"><h2 id="recent-sessions-heading" class="text-h2 font-semibold text-ink">Recent sessions</h2></header>
            <div class="space-y-stack-md pb-stack-md">
              ...
              <template v-else-if="recentSessions.length">
                <div v-for="session in recentSessions" :key="session.id" class="flex flex-col md:flex-row md:items-center justify-between p-inline-md border border-border rounded-control bg-surface-subtle gap-inline-md">
                  <div class="min-w-0">
                    <div class="flex items-center gap-inline-sm flex-wrap"><span class="font-medium text-ink">{{ formatSessionDate(session.startedAt) }}</span><span v-if="session.status === 'in_progress'" class="text-caption font-medium text-action-link">Open</span></div>
                  ...
                  <div class="flex items-center gap-inline-sm shrink-0">
                    ...
                    <button type="button" class="px-inline-md py-stack-xs border border-border bg-surface text-body-sm font-medium text-ink rounded-control hover:bg-surface-subtle" @click="openSessionWorkspace(session)">Open summary</button>
                    <button v-if="session.hasTranscript" type="button" class="px-inline-md py-stack-xs text-body-sm font-medium text-ink-secondary hover:text-ink" @click="viewTranscript(session)">Transcript</button>
                  </div>
                </div>
              </template>
          ...
          <section aria-labelledby="supporting-material-heading" class="pt-stack-xl border-t border-border-muted">
            ...
            <div class="space-y-stack-xl">
              <div id="section-care"><ClientCarePanel :client-id="String(route.params.clientId)" @changed="careRefreshKey++" /></div>
              <div id="section-timeline" class="bg-surface-elevated border border-border-muted rounded-panel p-inline-lg max-w-2xl mx-auto">
                ...
                <div v-if="timelineLoading" class="py-stack-xl text-center"><p class="text-body-sm text-ink-subtle">Loading clinical narrative...</p></div>
                <div v-else-if="timelineEvents.length === 0" class="py-stack-xl text-center"><p class="text-body-sm text-ink-subtle">No clinical events recorded for this client.</p></div>
                <div v-else class="space-y-0"><TimelineItem v-for="(event, index) in timelineEvents" :key="event.id" :event-type="event.event_type" :date="formatDate(event.occurred_at)" :description="event.summary" :subject-type="event.subject_type" :subject-id="event.subject_id" :session-id="event.session_id" :is-last="index === timelineEvents.length - 1" /></div>
              </div>
              <ClientDocumentsPanel :documents="documents" :loading="documentsLoading" :error="documentsError" :archived="client.archived" :focus-document-id="focusDocumentId" @create="newDocument" @edit="editDocument" @download="downloadDocument" />
              ...
              <section id="section-details" class="bg-surface-elevated border border-border-muted rounded-panel p-inline-lg"><h3 class="text-h3 font-semibold text-ink mb-stack-lg pt-stack-md">Client Details</h3><ClientDetails :client="client" @updated="client = $event" /></section>
            </div>
          </section>
        </div>
      </div>
...
import { listClientAppointments, nextClientAppointment } from '../lib/appointments.js';
import { downloadClientDocument, listClientDocuments } from '../lib/clientDocuments.js';
import { DEFAULT_LENS_ID } from '../lib/clinicalLenses.js';
...
import ClientCarePanel from '../components/workspace/ClientCarePanel.vue';
import ClientMeasuresPanel from '../components/workspace/ClientMeasuresPanel.vue';
import ClientResourcesPanel from '../components/workspace/ClientResourcesPanel.vue';
import CurrentCareFocus from '../components/workspace/CurrentCareFocus.vue';
import ClientFollowUps from '../components/workspace/ClientFollowUps.vue';
import TimelineItem from '../components/workspace/TimelineItem.vue';
import ClientDetails from './ClientDetails.vue';

const route=useRoute(),router=useRouter();
const client=ref(null),loading=ref(true),error=ref('');
const timelineEvents=ref([]),timelineLoading=ref(false);
const documents=ref([]),documentsLoading=ref(false),documentsError=ref(''),documentComposerOpen=ref(false),editingDocument=ref(null),newDocumentType=ref('other');
const sessions=ref([]),sessionsLoading=ref(false),appointments=ref([]),appointmentsLoading=ref(false),therapistLabel=ref('Current therapist');
const careRefreshKey=ref(0),selectedLensId=ref(DEFAULT_LENS_ID);
...
async function loadSessions(){sessionsLoading.value=true;try{sessions.value=await listSessions({clientId:route.params.clientId})}finally{sessionsLoading.value=false}}
...
function openSessionWorkspace(session){if(!session?.id||!client.value?.id)return;router.push({name:'SessionWorkspace',params:{clientId:client.value.id,sessionId:session.id}})}
function viewTranscript(session){if(!session?.id||!client.value?.id)return;router.push({name:'SessionWorkspace',params:{clientId:client.value.id,sessionId:session.id},query:{view:'transcript'}})}
function humanReadableState(session){if(session.status==='in_progress')return'Clinical workspace is open.';if(session.workflowStatus==='needs_review'||session.workflowStatus==='drafts_awaiting_review')return'Review is waiting.';return session.completedAt?'Completed.':'Recorded.'}
function scrollToSection(id){const el=document.getElementById(`section-${id}`);if(el)el.scrollIntoView({behavior:'smooth'})}
async function openRequestedDocument(){const documentId=focusDocumentId.value;if(!documentId)return;const requested=documents.value.find(document=>document.id===documentId);if(!requested)return;if(client.value?.archived&&requested.status!=='completed'){await nextTick();scrollToSection('documents');return}editingDocument.value=requested;newDocumentType.value=requested.documentType||'other';documentComposerOpen.value=true}
function newDocument(documentType='other'){if(client.value?.archived)return;editingDocument.value=null;newDocumentType.value=documentType||'other';documentComposerOpen.value=true}
function editDocument(document){if(client.value?.archived)return;editingDocument.value=document;newDocumentType.value=document.documentType||'other';documentComposerOpen.value=true}
async function downloadDocument(document){try{await downloadClientDocument(document)}catch(e){documentsError.value='Could not download this document.'}}
function closeDocumentComposer(){documentComposerOpen.value=false;editingDocument.value=null;newDocumentType.value='other'}
async function documentSaved(){await refreshDocuments()}
function showDocuments(){closeDocumentComposer();nextTick(()=>scrollToSection('documents'))}
...
function formatSessionDate(dateString){if(!dateString)return'';return new Date(dateString).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}
