<template>
  <div class="flex flex-col h-full bg-surface-canvas">
    ...
    <template v-else-if="client">
      <ClientWorkspaceHeader :client="client" :next-appointment="nextAppointment" :therapist-label="therapistLabel" @create-document="newDocument" @client-updated="client = $event" />
      ...
      <div class="flex-1 overflow-auto p-inline-lg py-stack-lg"><div class="max-w-6xl mx-auto space-y-stack-lg">
        ...
        <div v-if="activeTab === 'Overview'" class="space-y-stack-lg">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-stack-lg"><div class="lg:col-span-2 space-y-stack-lg">
            <section class="bg-surface-elevated border border-border-muted rounded-panel p-inline-lg"><h3 class="text-h3 font-semibold text-ink mb-stack-md pt-stack-md">Recent Sessions</h3><div class="space-y-stack-md pb-stack-md"><p v-if="sessionsLoading" class="text-body-sm text-ink-muted">Loading sessions…</p><template v-else-if="recentSessions.length"><div v-for="session in recentSessions" :key="session.id" class="flex flex-col p-inline-md border border-border rounded-control bg-surface-subtle"><div class="flex justify-between items-center"><span class="font-medium text-ink">Session</span><span class="text-caption text-ink-muted">{{ formatDate(session.startedAt) }}</span></div><p class="text-body-sm text-ink-secondary mt-1">{{ sessionSummary(session) }}</p></div></template><p v-else class="text-body-sm text-ink-muted">No sessions recorded for this client yet.</p></div></section>
            <section class="bg-surface-elevated border border-border-muted rounded-panel p-inline-lg"><h3 class="text-h3 font-semibold text-ink mb-stack-md pt-stack-md">Current Goals</h3><p class="text-body-sm text-ink-muted pb-stack-md">No structured goals are recorded for this client yet.</p></section>
...
const sessions = ref([]); const sessionsLoading = ref(false); const appointments = ref([]); const appointmentsLoading = ref(false); const therapistLabel = ref('Current therapist');
const recentSessions = computed(() => sessions.value.slice(0, 4)); const nextAppointment = computed(() => nextClientAppointment(appointments.value));
async function loadClient() { loading.value = true; error.value = ''; try { client.value = await getClient({ clientId: route.params.clientId }); const results = await Promise.allSettled([loadTimeline(), refreshDocuments(), loadSessions(), loadAppointments(), loadTherapist()]); for (const result of results) if (result.status === 'rejected') console.error('Client workspace context failed to load:', { code: result.reason?.code || 'CONTEXT_LOAD_FAILED' }); } catch (e) { console.error('Failed to load client:', { code: e?.code || 'CLIENT_LOAD_FAILED' }); error.value = 'The client workspace could not be loaded.'; } finally { loading.value = false; } }
async function loadTimeline() { const clientId = route.params.clientId; if (!clientId) return; timelineLoading.value = true; try { timelineEvents.value = await getTimelineEvents({ clientId }); } finally { timelineLoading.value = false; } }
...
async function downloadDocument(document) { try { await downloadClientDocument(document); } catch (e) { console.error('Failed to download document:', { code: e?.code || 'DOCUMENT_DOWNLOAD_FAILED' }); documentsError.value = 'Could not download this document.'; } }
