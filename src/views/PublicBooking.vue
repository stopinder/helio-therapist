...
onMounted(async () => { try { const token = encodeURIComponent(String(route.params.token || '')); const response = await fetch(`/api/booking?token=${token}`); const payload = await response.json().catch(() => ({})); state.value = payload.state || 'invalid'; if (response.ok && state.value === 'available') { bookingUrl.value = payload.bookingUrl || ''; expiresAt.value = payload.expiresAt || ''; therapist.value = payload.therapist || therapist.value } } catch { state.value = 'unavailable' } finally { loading.value = false } })
</script>
