    ...
    <main v-else class="flex-1 overflow-y-auto p-page">
      <div class="max-w-[68rem] mx-auto mb-stack-md">
        ...
        <section v-if="showPasteImport" class="mt-stack-md rounded-panel border border-border-muted bg-surface p-stack-lg" aria-label="Paste transcript">
          <div class="mb-stack-md"><h2 class="text-body font-semibold text-ink">Paste transcript</h2><p class="mt-stack-xs text-body-sm text-ink-muted">Copy the transcript from Zoom and paste it here. Helios will preserve it as the original source and will not analyse it automatically.</p></div>
          <label for="pasted-transcript" class="sr-only">Transcript text</label>
          <textarea id="pasted-transcript" v-model="pastedTranscript" class="control-field min-h-48 resize-y" placeholder="Paste the Zoom transcript here…" :disabled="importing"></textarea>
          <div class="mt-stack-md flex items-center justify-between gap-inline-md"><p class="text-caption text-ink-muted">Up to 2 MB. Re-importing the same transcript will not create a duplicate.</p><button type="button" class="button-primary shrink-0" :disabled="importing || !pastedTranscript.trim()" @click="importPastedTranscript">{{ importing ? 'Importing…' : 'Import pasted transcript' }}</button></div>
        </section>
      </div>
...
const openTranscriptId = computed(() => typeof route.query.transcript === 'string' ? route.query.transcript : null)
...
async function checkZoomNotes() {
  checkingZoom.value = true; importError.value = ''; importSuccess.value = ''
  try {
    ...
    if (data.imported > 0) { inboxKey.value += 1; importSuccess.value = `${data.imported} ${data.imported === 1 ? 'transcript' : 'transcripts'} imported from Zoom.` }
    else importSuccess.value = 'Zoom Notes checked. No missing transcripts found.'
  } catch (err) { importError.value = err.message || 'Unable to check Zoom Notes.' }
  finally { checkingZoom.value = false }
}

async function submitTranscriptImport(filename, text) {
  importError.value = ''; importSuccess.value = ''
  if (!text.trim()) { importError.value = 'Paste some transcript text before importing.'; return false }
  if (new Blob([text]).size > MAX_IMPORT_BYTES) { importError.value = 'That transcript is too large to import.'; return false }
  importing.value = true
  try {
    const response = await authenticatedFetch('/api/zoom/transcripts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename, text }) })
    const data = await response.json().catch(() => ({}))
    ...
    return true
  } catch (err) { console.error('[Transcript Import] Failed', err); importError.value = 'Couldn’t import the transcript. Please try again.'; return false }
  finally { importing.value = false }
}

async function importPastedTranscript() { const imported = await submitTranscriptImport('pasted-transcript.txt', pastedTranscript.value); if (imported) { pastedTranscript.value = ''; showPasteImport.value = false } }

async function importTranscriptFile(event) {
  const file = event.target.files?.[0]; event.target.value = ''; if (!file) return
  importError.value = ''; importSuccess.value = ''
  const extension = file.name.split('.').pop()?.toLowerCase()
  if (!['vtt', 'txt'].includes(extension)) { importError.value = 'Choose a Zoom transcript in .vtt or .txt format.'; return }
  if (file.size > MAX_IMPORT_BYTES) { importError.value = 'That transcript file is too large to import.'; return }
  const text = await file.text(); await submitTranscriptImport(file.name, text)
}
