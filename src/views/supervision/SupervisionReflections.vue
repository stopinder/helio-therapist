      ...
      <div class="p-6 md:p-8 border-b border-border-muted flex flex-col md:flex-row md:justify-between md:items-center gap-6 bg-surface">
        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div class="relative w-full md:w-48">
            ...
            >
              <option v-for="theme in themes" :key="theme.name" :value="theme.name">{{ theme.name }} ({{ theme.count }})</option>
            </select>
            <span class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-ink-muted text-xs">▼</span>
          </div>
        </div>
        <button v-if="searchQuery || selectedTheme !== 'All'" @click="clearFilters" class="text-caption font-semibold text-action-link hover:underline">Clear filters</button>
      </div>
...
const props = defineProps({
  ...
  loading: Boolean, actionLoading: [String, Number], themes: { type: Array, default: () => [] }
});
...
const filteredReflections = computed(() => props.reflections.filter(r => {
  const matchesTheme = selectedTheme.value === 'All' || (selectedTheme.value === 'No theme' ? !r.theme : r.theme === selectedTheme.value);
  const query = searchQuery.value.toLowerCase().trim();
  const matchesSearch = !query || (r.body && r.body.toLowerCase().includes(query)) || (r.theme && r.theme.toLowerCase().includes(query)) || (r.clients?.display_name && r.clients.display_name.toLowerCase().includes(query)) || (r.session_ref && r.session_ref.toLowerCase().includes(query));
  return matchesTheme && matchesSearch;
}));
function toggleMenu(id, event) { if (event) event.stopPropagation(); menuOpenFor.value = menuOpenFor.value === id ? null : id; }
function clearFilters() { searchQuery.value = ''; selectedTheme.value = 'All'; }
</script>
