    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-9 items-start">
      <section class="lg:col-span-2 space-y-4">
        <SectionHeader title="Today’s schedule">
          <template #action><router-link to="/calendar" class="type-ui text-action-link hover:text-action-link-hover underline-offset-4 hover:underline">View calendar →</router-link></template>
        </SectionHeader>
          ...
          <SurfaceCard v-for="event in todayEvents" :key="event.id" compact class="flex flex-wrap md:flex-nowrap items-center justify-between gap-4 group">
            <div class="flex items-center gap-4 min-w-0">
              <div class="w-[4.25rem] shrink-0 pr-4 border-r border-border-muted">
                <div class="type-body-medium text-ink">{{ formatTime(event.start) }}</div>
                <div class="type-metadata text-ink-muted mt-1">60 min</div>
              </div>
              <div class="min-w-0">
                <div class="type-body-medium text-ink truncate">{{ event.clientName }}</div>
                <div class="type-ui text-ink-secondary mt-0.5">{{ event.type }}</div>
                <StatusIndicator class="mt-0.5">{{ formatStatus(event.status) }}</StatusIndicator>
              </div>
            </div>
            <div class="flex items-center gap-2 ml-auto">
              <AppButton v-if="event.clientId" :to="`/clients/${event.clientId}`">Open client</AppButton>
              <AppButton v-if="event.isEligibleForStart" variant="primary" :disabled="startingEventId === event.id" :aria-busy="startingEventId === event.id" @click="startSession(event)">{{ startingEventId === event.id ? 'Opening…' : 'Start session' }}</AppButton>
            </div>
      ...
      <aside class="space-y-6">
        <section class="space-y-3">
          ...
          <SurfaceCard v-if="pendingWork.length === 0" tone="base" compact class="text-center"><p class="type-metadata text-ink-subtle">No pending drafts or reviews.</p></SurfaceCard>
          <div v-else class="space-y-2.5"><router-link v-for="item in pendingWork" :key="item.id" :to="item.route" class="block focus-visible:rounded-panel"><SurfaceCard compact><div class="type-ui font-semibold text-ink">{{ item.title }}</div><div class="type-metadata text-ink-muted mt-1">{{ item.subtitle }}</div></SurfaceCard></router-link></div>
        </section>
        ...
        <section class="space-y-3"><p class="type-eyebrow text-ink-muted">Recent activity</p><SurfaceCard tone="muted" compact class="text-center"><p class="type-metadata text-ink-subtle">No recent activity to show.</p></SurfaceCard></section>
      </aside>
    </div>
