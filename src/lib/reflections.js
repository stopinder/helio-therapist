...
export async function getPrivateReflection({ supabaseClient, clientId, sessionId }) {
  if (!supabaseClient && !supabase) { const module = await import('./supabase.js'); supabase = module.supabase; }
  const client = supabaseClient || supabase;
  if (!client) throw new Error('Supabase client not initialized');
  const { data: { user } } = await client.auth.getUser();
  if (!user) throw new Error('Not authenticated');
...
export async function getPrivateReflectionsForClient({ supabaseClient, clientId }) {
  ...
  const { data, error } = await client.from('private_reflections')
    ...
    .eq('user_id', user.id).eq('client_id', clientId).order('created_at', { ascending: false });
  if (error) {
    console.error('[Reflections] Fetch client reflections error:', error);
...
export async function upsertPrivateReflection({ supabaseClient, clientId, sessionId, body }) {
  ...
  if (existing) {
    const { data, error } = await client.from('private_reflections').update({ body, updated_at: new Date().toISOString() }).eq('id', existing.id).select().single();
    if (error) { console.error('[Reflections] Update error:', error); throw new Error(error.message || 'Could not update private reflection'); }
    return data;
  }
  const { data, error } = await client.from('private_reflections').insert({ user_id: user.id, client_id: clientId, session_ref: sessionId, body }).select().single();
