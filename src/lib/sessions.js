...
export function presentClinicalRecordAmendment(row) {
  if (!row) return null
  return {
    id: row.id,
...
export async function listClinicalRecordAmendments(sessionId) {
  ...
  if (error) throw error
  return (data || []).map(presentClinicalRecordAmendment)
}
...
export async function approveClinicalRecordAmendment({ sessionId, reason, content }) {
  ...
  if (error) throw error
  return presentClinicalRecordAmendment(singleResult(data))
}