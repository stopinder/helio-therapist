    ...
    <div class="flex flex-col flex-1 min-w-0 min-h-0 h-full overflow-hidden">
      <header class="h-16 flex items-center justify-between px-inline-lg md:px-6 border-b border-border-muted bg-surface shrink-0">
        ...
        <div class="flex items-center gap-2 md:gap-inline-md">
          ...
          <router-link v-if="$route.path!=='/schedule'" to="/schedule" :class="['hidden sm:inline-flex items-center gap-2', hasContextualPrimary ? 'button-secondary' : 'button-primary']"><span class="leading-none font-normal">+</span>Schedule appointment</router-link>
        </div>
      </header>
...
const now=ref(new Date()),appointments=ref([]);let clockTimer;
