  ...
  <div class="flex h-screen bg-surface-canvas text-ink overflow-hidden">
    ...
    <div class="flex flex-col flex-1 min-w-0 min-h-0 h-full overflow-hidden">
      <header class="h-16 flex items-center justify-between px-inline-lg md:px-6 border-b border-border-muted bg-surface shrink-0">
        <div class="flex items-center gap-inline-md"><button class="md:hidden p-2 -ml-2 text-ink-secondary hover:bg-surface-subtle rounded-control" @click="isMobileMenuOpen=true" aria-label="Open menu"><Bars3Icon class="h-5 w-5" /></button><h2 class="font-serif text-h2 font-semibold text-ink truncate">{{ currentPageName }}</h2></div>
        <div class="flex items-center gap-inline-md"><router-link v-if="$route.path!=='/schedule'" to="/schedule" class="hidden sm:inline-flex items-center gap-2 rounded-control bg-action-primary text-on-action px-4 py-2.5 text-body-sm font-semibold hover:bg-action-primary-hover shadow-sm"><span class="text-lg leading-none font-normal">+</span>Schedule appointment</router-link><div v-if="isSessionActive" class="flex items-center gap-2 px-3 py-2 rounded-pill bg-surface-elevated border border-border-muted shadow-sm"><span class="h-2 w-2 rounded-pill bg-state-success"></span><span class="text-caption font-medium text-ink-secondary">Workspace active</span></div></div>
      </header>
...
onMounted(()=>{window.addEventListener('helios-session-expired',handleExpiry);loadAccountIdentity()});onUnmounted(()=>window.removeEventListener('helios-session-expired',handleExpiry));const handleSignOut=async()=>{if(supabase)await supabase.auth.signOut()};
...
const BrandLockup=defineComponent({name:'BrandLockup',setup(){return()=>h('div',{class:'flex items-center gap-2.5'},[h('div',{class:'relative h-8 w-8 rounded-pill border border-brand-amber/40 flex items-center justify-center brand-accent-surface'},[h('span',{class:'brand-mark text-lg leading-none'},'☼')]),h('div',{class:'leading-none'},[h('p',{class:'font-serif text-[1.45rem] font-semibold tracking-[-0.025em] text-ink'},'Helios'),h('p',{class:'text-[0.58rem] uppercase tracking-[0.16em] text-ink-muted mt-1'},'Practice')])])}});
const SidebarNavigation=defineComponent({name:'SidebarNavigation',props:{groups:{type:Array,required:true},currentPath:{type:String,required:true}},emits:['navigate'],setup(props,{emit}){const active=item=>item.path==='/'?props.currentPath==='/':props.currentPath===item.path||props.currentPath.startsWith(`${item.path}/`);return()=>h('nav',{class:'sidebar-navigation flex-1 min-h-0 overflow-y-auto px-3 py-5 space-y-6'},props.groups.map(group=>h('section',{key:group.label},[h('p',{class:'px-3 mb-2 type-overline text-ink-subtle'},group.label),h('div',{class:'space-y-1'},group.items.map(item=>h(RouterLink,{key:item.name,to:item.path,onClick:()=>emit('navigate'),class:['flex items-center gap-3 px-3 py-2.5 rounded-control text-body-sm transition-colors duration-standard ease-out',active(item)?'bg-state-selected text-ink font-semibold shadow-sm':'text-ink-secondary hover:bg-surface-subtle hover:text-ink']},()=>[h(item.icon,{class:['h-[1.1rem] w-[1.1rem] shrink-0',active(item)?'text-action-primary':'text-ink-muted']}),h('span',item.name)])))])))} });
const SidebarAccount=defineComponent({name:'SidebarAccount',props:{account:{type:Object,required:true}},emits:['sign-out'],setup(props,{emit}){return()=>h('div',{class:'shrink-0 border-t border-border-muted px-3 py-3 bg-sidebar'},[h('div',{class:'flex items-center gap-3 px-2 py-2 min-w-0'},[h('div',{class:'h-9 w-9 rounded-pill bg-action-primary flex items-center justify-center text-caption font-semibold text-on-action shrink-0'},props.account.initials),h('div',{class:'min-w-0 leading-tight'},[h('p',{class:'text-body-sm font-semibold text-ink truncate'},props.account.name),props.account.subtitle?h('p',{class:'text-caption text-ink-muted truncate mt-1'},props.account.subtitle):null])]),h(RouterLink,{to:'/settings',class:'mt-1 flex items-center gap-2.5 w-full px-2 py-2 rounded-control text-caption font-medium text-ink-muted hover:text-ink-secondary hover:bg-surface-subtle'},()=>[h(Cog6ToothIcon,{class:'h-4 w-4'}),h('span','Settings')]),h('button',{type:'button',onClick:()=>emit('sign-out'),class:'flex items-center gap-2.5 w-full px-2 py-2 rounded-control text-caption text-ink-subtle hover:text-ink-secondary hover:bg-surface-subtle'},[h(ArrowRightStartOnRectangleIcon,{class:'h-4 w-4'}),h('span','Sign out')])])}});
</script>
<style scoped>
