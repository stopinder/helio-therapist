...
import { AcademicCapIcon,ArrowRightStartOnRectangleIcon,Bars3Icon,CalendarDaysIcon,Cog6ToothIcon,DocumentTextIcon,EllipsisHorizontalIcon,FolderOpenIcon,PlusIcon,Squares2X2Icon,SunIcon,UsersIcon,XMarkIcon } from '@heroicons/vue/24/outline';
...
onMounted(()=>{window.addEventListener('helios-reminders-changed',handleReminderChange);window.addEventListener('focus',refreshAppointments);document.addEventListener('visibilitychange',refreshAppointmentsWhenActive);loadAccountIdentity();refreshReminders();refreshAppointments();clockTimer=window.setInterval(()=>{now.value=new Date()},30000);appointmentRefreshTimer=window.setInterval(refreshAppointments,60000)});
onUnmounted(()=>{window.removeEventListener('helios-reminders-changed',handleReminderChange);window.removeEventListener('focus',refreshAppointments);document.removeEventListener('visibilitychange',refreshAppointmentsWhenActive);window.clearInterval(clockTimer);window.clearInterval(appointmentRefreshTimer)});const handleSignOut=async()=>{if(supabase)await supabase.auth.signOut();await router.replace('/sign-in')};
const navGroups=[{label:'Practice',items:[{name:'Overview',path:'/overview',icon:Squares2X2Icon},{name:'Calendar',path:'/calendar',icon:CalendarDaysIcon},{name:'Clients',path:'/clients',icon:UsersIcon}]},{label:'Records',items:[{name:'Transcripts',path:'/transcripts',icon:DocumentTextIcon},{name:'Documents',path:'/documents',icon:FolderOpenIcon}]},{label:'Professional',items:[{name:'CPD',path:'/supervision',icon:AcademicCapIcon}]}];
const pageTitles={'/overview':'Overview','/calendar':'Calendar','/schedule':'Schedule appointment','/clients':'Clients','/transcripts':'Transcripts','/documents':'Documents','/supervision':'CPD','/settings':'Settings'};const currentPageName=computed(()=>pageTitles[route.path]||route.meta?.title||'Workspace');
const BrandLockup=defineComponent({name:'BrandLockup',setup(){return()=>h('div',{class:'flex items-center gap-2.5'},[h('div',{class:'relative h-8 w-8 rounded-pill border border-brand-amber/40 flex items-center justify-center brand-accent-surface'},[h(SunIcon,{class:'h-5 w-5 text-focus','aria-hidden':'true'})]),h('div',{class:'leading-none'},[h('p',{class:'font-serif text-[1.35rem] font-semibold tracking-[-0.02em] text-ink'},'Helios'),h('p',{class:'text-[0.58rem] uppercase tracking-[0.16em] text-ink-muted mt-1'},'Practice')])])}});
const SidebarNavigation=defineComponent({name:'SidebarNavigation',props:{groups:{type:Array,required:true},currentPath:{type:String,required:true}},emits:['navigate'],setup(props,{emit}){const active=item=>props.currentPath===item.path||props.currentPath.startsWith(`${item.path}/`);return()=>h('nav',{class:'sidebar-navigation flex-1 min-h-0 overflow-y-auto px-3 py-4 space-y-5'},props.groups.map(group=>h('section',{key:group.label},[h('p',{class:'px-3 mb-2 type-eyebrow text-ink-subtle'},group.label),h('div',{class:'space-y-1'},group.items.map(item=>h(RouterLink,{key:item.name,to:item.path,onClick:()=>emit('navigate'),class:['flex items-center gap-3 min-h-touch px-3 rounded-control type-ui transition-colors duration-standard ease-out',active(item)?'bg-state-selected text-ink font-semibold':'text-ink-secondary hover:bg-surface-subtle hover:text-ink']},()=>[h(item.icon,{class:['h-[1.1rem] w-[1.1rem] shrink-0',active(item)?'text-action-primary':'text-ink-muted']}),h('span',item.name)])))])))} });
