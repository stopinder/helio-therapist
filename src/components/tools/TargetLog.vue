...
<template>
  <div class="p-6 bg-surface-elevated rounded-panel  border border-border max-w-4xl mx-auto">
    <h2 class="text-h3 font-semibold text-ink mb-3">EMDR Target Log</h2>
    ...
    <ul class="divide-y divide-border-muted text-body">
      <li v-for="(t, idx) in targets" :key="idx" class="py-3">
        <div class="flex justify-between items-start">
          <div class="flex-1">
            <div class="font-semibold text-ink">{{ t.memory }}</div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-2">
              <div>
                <label class="block text-caption text-ink-muted mb-0.5">SUD (0–10)</label>
                <input
                    v-model="t.sud"
                    type="number"
                    min="0"
              ...
              <div>
                <label class="block text-caption text-ink-muted mb-0.5">VOC (1–7)</label>
                <input
                    v-model="t.voc"
                    type="number"
                    min="1"
