...
<template>
  <div class="p-6 bg-white rounded-lg shadow-sm border border-[#d9dce1] max-w-4xl mx-auto">
    <h2 class="text-h3 font-semibold text-[#2c3e50] mb-3">Cognitive Interweave</h2>
    ...
    <div class="flex items-center gap-2 mb-4">
      <input
          ...
          class="flex-1 border border-[#d9dce1] rounded-md px-3 py-1.5 text-body"
      />
      <input
      ...
      <button
          ...
          class="px-3 py-1.5 bg-[#3f4754] text-white rounded-md text-body-sm hover:bg-[#2f3540]"
      >
        Add
      </button>
    </div>
    ...
    <!-- Interweave list -->
    <ul class="divide-y divide-[#e5e7eb] text-body">
      <li v-for="(iw, idx) in interweaves" :key="idx" class="py-3">
        <div class="flex justify-between items-start">
          <div class="flex-1">
            <div class="font-semibold text-[#2c3e50]">{{ iw.target }}</div>
            <div class="text-slate-600 text-body-sm mb-1">
              Purpose: {{ iw.purpose }}
            </div>

            <label class="block text-caption text-slate-500 mb-0.5">Cognition or Prompt</label>
            <input
                v-model="iw.cognition"
                placeholder="Therapist question or reframe"
                class="w-full border border-[#d9dce1] rounded-md px-2 py-1 text-body-sm mb-2"
            />

            <label class="block text-caption text-slate-500 mb-0.5">Outcome / Notes</label>
            <textarea
                v-model="iw.notes"
                placeholder="Describe client response or session effect..."
                rows="2"
                class="w-full border border-[#d9dce1] rounded-md px-2 py-1 text-body-sm"
            ></textarea>
          </div>

          <button
              v-on:click="removeInterweave(idx)"
              class="text-body-sm text-red-600 hover:underline ml-3 mt-1"
          >
            ✕
          </button>
        </div>
