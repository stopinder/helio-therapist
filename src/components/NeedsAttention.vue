...
async function load() { loading.value = true; error.value = ''; try { const [transcriptResponse, assignmentResponse, sessionData] = await Promise.all([authenticatedFetch('/api/zoom/transcripts'), authenticatedFetch('/api/resource-assignments?needsAttention=true'), listSessions()]); const transcriptData = await transcriptResponse.json().catch(() => ({})); const assignmentData = await assignmentResponse.json().catch(() => ({})); if (!transcriptResponse.ok) throw new Error(transcriptData.error || 'Unable to load the work queue.'); if (!assignmentResponse.ok) throw new Error(assignmentData.error || 'Unable to load client review work.'); transcripts.value = transcriptData.transcripts || []; assignments.value = assignmentData.assignments || []; sessions.value = sessionData } catch (err) { error.value = err.message || 'Unable to load the work queue.' } finally { loading.value = false } }
...
<style scoped>
.attention-workspace{max-width:68rem;margin:0 auto;color:var(--text-primary)}.attention-workspace>header{display:flex;align-items:flex-start;justify-content:space-between;gap:var(--space-inline-lg);margin-bottom:var(--space-stack-xl)}.eyebrow,.item-type{margin:0 0 var(--space-stack-xs);text-transform:uppercase;letter-spacing:.08em;color:var(--text-muted);font-size:.72rem;font-weight:700}.attention-workspace h1{font-size:1.7rem;margin:0}.attention-workspace>header p:not(.eyebrow){margin:var(--space-stack-xs) 0;color:var(--text-muted)}.refresh,.primary{border-radius:.6rem;padding:.65rem .85rem;font-weight:700}.refresh{border:1px solid var(--border);background:var(--surface-elevated);color:var(--text-secondary)}.primary{border:1px solid var(--action-link);background:var(--action-link);color:var(--text-on-action);white-space:nowrap}.primary span{font-size:1.1rem}.notice{padding:var(--space-stack-md);border-radius:.65rem;background:var(--state-danger-surface);color:var(--state-danger)}.empty-card{min-height:14rem;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;background:var(--surface-elevated);border:1px solid var(--border-muted);border-radius:.9rem;color:var(--text-muted);padding:var(--space-stack-2xl)}.empty-card div{display:grid;place-items:center;width:3rem;height:3rem;border-radius:50%;background:var(--state-success-surface);color:var(--state-success);font-size:1.5rem;font-weight:700}.empty-card h2{color:var(--text-secondary);margin:var(--space-stack-md) 0 var(--space-stack-xs)}.queue{display:grid;gap:var(--space-stack-sm)}.queue-item{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:var(--space-inline-lg);background:var(--surface-elevated);border:1px solid var(--border-muted);border-radius:.8rem;padding:var(--space-stack-lg) var(--space-stack-xl)}.queue-item:hover{border-color:var(--state-selected);box-shadow:none}.item-icon{display:grid;place-items:center;width:2.5rem;height:2.5rem;border-radius:.65rem;background:var(--state-selected);color:var(--action-link-hover)}.item-copy h2{font-size:1rem;margin:0}.item-copy p:not(.item-type),.item-copy small{margin:var(--space-stack-xs) 0 0;color:var(--text-muted);line-height:1.4}.item-copy small{display:block;font-weight:700;color:var(--text-secondary)}@media(max-width:650px){.attention-workspace>header{align-items:stretch;flex-direction:column}.queue-item{grid-template-columns:auto 1fr}.queue-item .primary{grid-column:1/-1;width:100%}.refresh{align-self:flex-start}}
.review-backdrop{position:fixed;inset:0;z-index:90;background:rgb(24 32 28 / .28);display:flex;align-items:center;justify-content:center;padding:1rem}.review-dialog{width:min(42rem,100%);max-height:90vh;overflow:auto;background:var(--surface-overlay);border:1px solid var(--border);border-radius:1rem;padding:1.3rem;box-shadow:var(--shadow-overlay)}.review-dialog h2{margin:.15rem 0 .7rem}.instruction,.score{padding:.75rem;border:1px solid var(--border-muted);background:var(--surface-muted);border-radius:.6rem}.quiet{color:var(--text-muted);line-height:1.45}.answers{padding-left:1.2rem}.answers li{padding:.75rem .1rem;border-bottom:1px solid var(--border-muted)}.answers strong,.answers span{display:block}.answers span{margin-top:.25rem;color:var(--text-secondary)}.review-dialog footer{display:flex;justify-content:flex-end;gap:.5rem;margin-top:1rem}

/* Queue items are operational panels, not floating cards. */
.refresh{border-color:var(--border);background:var(--surface-elevated)}.refresh:hover{background:var(--surface-subtle);border-color:var(--border-strong)}.refresh:focus-visible,.primary:focus-visible{outline:2px solid var(--action-link);outline-offset:2px}.empty-card{background:var(--surface);border-color:var(--border-muted)}.queue{gap:0;border-top:1px solid var(--border-muted);border-bottom:1px solid var(--border-muted);background:var(--surface)}.queue-item{background:var(--surface);border:0;border-bottom:1px solid var(--border-muted);border-radius:0}.queue-item:last-child{border-bottom:0}.queue-item:hover{background:var(--surface-subtle);box-shadow:none}.item-icon{background:var(--surface-muted);color:var(--action-link-hover)}
</style>
