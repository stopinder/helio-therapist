<template>
  <section class="attention-workspace">
    <header>
      <div><p class="eyebrow">Today</p><h1>Needs attention</h1><p>Only work that has a clear next action appears here.</p></div>
      <button class="refresh" :disabled="loading" @click="load">{{ loading ? 'Refreshing…' : 'Refresh' }}</button>
    </header>
...
const emit = defineEmits(['open-transcript', 'open-session'])
