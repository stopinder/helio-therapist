<template>
  <section class="max-w-3xl mx-auto px-4 py-8 md:px-8 md:py-10" data-testid="schedule-appointment-workspace">
    ...
    <div class="rounded-xl border border-border bg-surface p-5 md:p-6">
      ...
      <select id="schedule-client" v-model="clientId" class="w-full rounded-control border border-border bg-surface-elevated px-3 py-2.5 text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-state-focus-ring">
        <option value="">Choose a client</option>
        <option v-for="client in clients" :key="client.id" :value="client.id">{{ client.name }}</option>
      </select>

      <p v-if="!clients.length" class="type-body-sm text-ink-muted mt-3">Add an active client before scheduling an appointment.</p>
      ...
      <div class="mt-6 flex items-center gap-3">
        <button type="button" class="rounded-control bg-action-primary text-on-action px-4 py-2.5 type-body-sm font-semibold hover:bg-action-primary-hover disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!clientId || loading" @click="createBookingLink">
          {{ loading ? 'Creating link…' : 'Continue to available times' }}
        </button>
...
const selectedClient = computed(() => props.clients.find(client => String(client.id) === String(clientId.value)) || null)
...
async function createBookingLink() {
  ...
  try {
    ...
    error.value = cause.message || 'Unable to create booking link'
  } finally {
    loading.value = false
  }
}
