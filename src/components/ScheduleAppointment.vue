  ...
  <section class="max-w-3xl mx-auto px-4 py-8 md:px-8 md:py-10" data-testid="schedule-appointment-workspace">
    ...
    <div class="rounded-xl border border-border bg-surface p-5 md:p-6">
      ...
      <div v-if="clientId && !bookingUrl" class="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <button type="button" class="rounded-xl border border-border p-4 text-left hover:border-action-link disabled:opacity-50" :disabled="loading" @click="createBookingLink('therapist')"><span class="block type-body-sm font-semibold text-ink">Choose time myself</span><span class="block type-body-sm text-ink-secondary mt-1">Open the same secure booking page yourself and select the time for the client.</span></button>
      </div>
      <p v-if="loading" class="type-body-sm text-ink-muted mt-4">Creating secure booking link…</p>
      <div v-if="bookingUrl" class="mt-5 rounded-lg bg-surface-subtle border border-border-muted p-4" role="status">
        ...
        <div v-if="mode === 'client'" class="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        ...
        <div v-else class="mt-3"><a :href="bookingUrl" target="_blank" rel="noopener" class="inline-flex justify-center rounded-control bg-action-primary text-on-action px-4 py-2 type-body-sm font-semibold">Choose available time ↗</a></div>
        <p v-if="mode === 'client' && !selectedClient?.email" class="type-caption text-ink-muted mt-3">No email address is saved for this client. Copy the link and send it using your usual contact method.</p>
