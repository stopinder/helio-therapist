...
async function saveProfile(){isSavingProfile.value=true;profileError.value='';try{const user=await currentUser();const {data,error}=await supabase.from('profiles').update({...profile.value}).eq('id',user.id).select('id').maybeSingle();if(error)throw error;if(!data)throw new Error('Your therapist profile could not be found. Please sign in again.')}catch(error){profileError.value=error.message||'Unable to save your therapist profile.'}finally{isSavingProfile.value=false}}
