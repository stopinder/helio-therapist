<template>
  <div class="max-w-3xl mx-auto py-6 sm:py-10 px-4 sm:px-6 w-full overflow-x-hidden">
    ...
    <section class="mb-12" data-testid="settings-practice-profile">
      ...
      <form class="bg-surface-elevated border border-border-muted rounded-panel p-4 sm:p-5 space-y-6" @submit.prevent="saveProfile" @input="profileSuccess = ''">
        <div>
        ...
        <p class="text-caption text-ink-subtle">Your practice contact details are optional. Add only the information you want Helios to use on documents.</p>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <label class="block"><span class="text-body-sm font-medium text-ink-secondary">Professional title <span class="font-normal text-ink-subtle">(optional)</span></span><input v-model.trim="profile.professional_title" type="text" placeholder="Psychotherapist" class="mt-2 min-h-touch w-full rounded-control border border-border bg-surface px-3" /></label>
          <label class="block sm:col-span-2"><span class="text-body-sm font-medium text-ink-secondary">Practice name <span class="font-normal text-ink-subtle">(optional)</span></span><input v-model.trim="profile.practice_name" type="text" autocomplete="organization" class="mt-2 min-h-touch w-full rounded-control border border-border bg-surface px-3" /></label>
          <label class="block"><span class="text-body-sm font-medium text-ink-secondary">Practice email <span class="font-normal text-ink-subtle">(optional)</span></span><input v-model.trim="profile.document_email" type="email" autocomplete="email" class="mt-2 min-h-touch w-full rounded-control border border-border bg-surface px-3" /></label>
          <label class="block"><span class="text-body-sm font-medium text-ink-secondary">Practice phone <span class="font-normal text-ink-subtle">(optional)</span></span><input v-model.trim="profile.document_phone" type="tel" autocomplete="tel" class="mt-2 min-h-touch w-full rounded-control border border-border bg-surface px-3" /></label>
          <label class="block sm:col-span-2"><span class="text-body-sm font-medium text-ink-secondary">Practice website <span class="font-normal text-ink-subtle">(optional)</span></span><input v-model.trim="profile.practice_website" type="url" autocomplete="url" placeholder="https://" class="mt-2 min-h-touch w-full rounded-control border border-border bg-surface px-3" /></label>
          <label class="block sm:col-span-2"><span class="text-body-sm font-medium text-ink-secondary">Practice address <span class="font-normal text-ink-subtle">(optional)</span></span><textarea v-model.trim="profile.practice_address" rows="3" autocomplete="street-address" class="mt-2 w-full rounded-control border border-border bg-surface px-3 py-2.5"></textarea></label>
        </div>
        <div class="flex flex-wrap items-center gap-3"><button type="submit" :disabled="isSavingProfile || isLoadingProfile" class="min-h-touch px-4 py-2 rounded-control bg-action-link text-on-action text-body-sm font-medium disabled:opacity-50">{{ isSavingProfile ? 'Saving…' : 'Save details' }}</button><span v-if="profileSuccess" role="status" aria-live="polite" class="text-body-sm text-state-success">{{ profileSuccess }}</span><span v-if="profileError" role="alert" class="text-body-sm text-state-danger">{{ profileError }}</span></div>
      </form>
    </section>
...
const profile=ref(emptyProfile()),isLoadingProfile=ref(true),isSavingProfile=ref(false),profileError=ref(''),profileSuccess=ref('')
...
async function saveProfile(){isSavingProfile.value=true;profileError.value='';profileSuccess.value='';try{const user=await currentUser();const {data,error}=await supabase.from('profiles').update({...profile.value}).eq('id',user.id).select('id').maybeSingle();if(error)throw error;if(!data)throw new Error('Your therapist profile could not be found. Please sign in again.');profileSuccess.value='Saved to your practice profile. These details will be used on Helios documents where practice details are included.'}catch(error){profileError.value=error.message||'Unable to save your therapist profile.'}finally{isSavingProfile.value=false}}
