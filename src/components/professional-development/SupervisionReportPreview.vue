  ...
  >
    <div class="w-full max-w-5xl bg-surface rounded-panel shadow-overlay max-h-[90vh] flex flex-col overflow-hidden border border-border">
      <div class="p-6 border-b border-border-muted flex justify-between items-center bg-surface-elevated">
        ...
        <button @click="$emit('close')" class="p-2 text-ink-muted hover:text-ink transition-colors rounded-control hover:bg-surface-subtle" aria-label="Close export preview"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12" /></svg></button>
      </div>
      <div class="flex-1 flex flex-col lg:flex-row overflow-hidden bg-surface-subtle">
        <div class="w-full lg:w-72 border-r border-border-muted bg-surface p-6 overflow-y-auto">
          <div class="space-y-8">
            <div><h3 class="type-overline text-ink-muted mb-4">Content Options</h3><div class="space-y-4">
              <label class="flex items-center gap-3 cursor-pointer group"><div class="relative inline-block w-8 h-4 transition duration-200 ease-in-out bg-surface-muted border border-border-muted rounded-full"><input type="checkbox" :checked="options.includeText" @change="$emit('update-option', 'includeText', $event.target.checked)" class="absolute block w-3 h-3 mt-0.5 ml-0.5 bg-white border border-border-muted rounded-full appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-4 checked:bg-action-primary" /></div><span class="text-caption font-medium text-ink group-hover:text-action-primary transition-colors">Include reflection text</span></label>
              <label class="flex items-center gap-3 cursor-pointer group"><div class="relative inline-block w-8 h-4 transition duration-200 ease-in-out bg-surface-muted border border-border-muted rounded-full"><input type="checkbox" :checked="options.includeThemes" @change="$emit('update-option', 'includeThemes', $event.target.checked)" class="absolute block w-3 h-3 mt-0.5 ml-0.5 bg-white border border-border-muted rounded-full appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-4 checked:bg-action-primary" /></div><span class="text-caption font-medium text-ink group-hover:text-action-primary transition-colors">Include themes</span></label>
              <label class="flex items-center gap-3 cursor-pointer group"><div class="relative inline-block w-8 h-4 transition duration-200 ease-in-out bg-surface-muted border border-border-muted rounded-full"><input type="checkbox" :checked="options.includeDates" @change="$emit('update-option', 'includeDates', $event.target.checked)" class="absolute block w-3 h-3 mt-0.5 ml-0.5 bg-white border border-border-muted rounded-full appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-4 checked:bg-action-primary" /></div><span class="text-caption font-medium text-ink group-hover:text-action-primary transition-colors">Include dates</span></label>
            </div></div>
            <div><h3 class="type-overline text-ink-muted mb-4">Privacy Options</h3><div class="space-y-4"><label class="flex items-center gap-3 cursor-pointer group"><div class="relative inline-block w-8 h-4 transition duration-200 ease-in-out bg-surface-muted border border-border-muted rounded-full"><input type="checkbox" :checked="options.includeClientReferences" @change="$emit('update-option', 'includeClientReferences', $event.target.checked)" class="absolute block w-3 h-3 mt-0.5 ml-0.5 bg-white border border-border-muted rounded-full appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-4 checked:bg-action-primary" /></div><span class="text-caption font-medium text-ink group-hover:text-action-primary transition-colors">Include client names</span></label><div class="text-caption text-ink-muted bg-surface-subtle p-3 rounded-panel border border-border-muted">Client names are excluded by default. Case aliases are used to maintain privacy.</div></div></div>
            <div><h3 class="type-overline text-ink-muted mb-2">Introduction</h3><textarea :value="introduction" @input="$emit('update-introduction', $event.target.value)" rows="4" placeholder="Add a brief introduction to your report..." class="w-full p-3 bg-surface border border-border rounded-control text-caption focus:ring-2 focus:ring-action-primary outline-none transition-all resize-none"></textarea></div>
          </div>
        </div>
