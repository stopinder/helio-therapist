  ...
  <div class="space-y-stack-lg max-w-4xl mx-auto" data-testid="completed-clinical-record">
    <div class="bg-surface-elevated border border-border-muted rounded-panel p-6 shadow-sm">
      ...
      <div v-if="legacyNotes" class="mt-8 pt-8 border-t border-border">
        ...
        <div class="p-4 rounded-control border border-border bg-surface-muted text-body-sm text-ink whitespace-pre-wrap leading-relaxed">{{ legacyNotes }}</div>
      </div>
    ...
    <div v-if="editing" class="bg-surface-elevated border border-border-muted rounded-panel p-6 shadow-sm" data-testid="clinical-record-amendment-editor">
      ...
      <div class="space-y-6">
        <label class="block"><span class="text-body-sm font-bold text-ink uppercase tracking-wider">Amendment Reason</span><textarea v-model="draft.reason" rows="2" class="mt-2 w-full p-3 rounded-control border border-border bg-surface-reflection focus:ring-2 focus:ring-state-reflection-focus outline-none" placeholder="Why is this amendment needed?" /></label>
        <label class="block"><span class="text-body-sm font-bold text-ink uppercase tracking-wider">Amendment Content</span><textarea v-model="draft.content" rows="6" class="mt-2 w-full p-3 rounded-control border border-border bg-surface focus:ring-2 focus:ring-action-link outline-none" placeholder="Enter the correction or additional clinical context." /></label>
      </div>
      <label class="mt-6 flex items-start gap-3"><input v-model="reviewConfirmed" type="checkbox" class="mt-1 h-4 w-4 rounded border-border text-action-link focus:ring-state-focus-ring"><span class="text-body-sm text-ink">I have reviewed this amendment and confirm it should be appended to the clinical record.</span></label>
      ...
      <div class="mt-8 pt-6 border-t border-border flex justify-end gap-3">
        <button type="button" class="px-inline-lg py-stack-sm bg-surface border border-border text-body-sm font-medium text-ink rounded-control" :disabled="saving" @click="cancelAmendment">Cancel</button>
        <button type="button" class="px-inline-lg py-stack-sm bg-action-primary text-on-action text-body-sm font-bold rounded-control disabled:opacity-50" :disabled="saving || !canApprove" @click="approveAmendment">{{ saving ? 'Approving…' : 'Approve Amendment' }}</button>
      </div>
    </div>
...
function startAmendment(){ draft.reason=''; draft.content=''; reviewConfirmed.value=false; saveError.value=''; editing.value=true }
function cancelAmendment(){ if(saving.value)return; editing.value=false }
async function approveAmendment(){ if(!canApprove.value || saving.value)return; saving.value=true; saveError.value=''; try { const saved=await approveClinicalRecordAmendment(props.session.id,{ reason:draft.reason, content:draft.content }); amendments.value=[...amendments.value,saved].sort((a,b)=>a.sequenceNumber-b.sequenceNumber); editing.value=false } catch { saveError.value='The amendment could not be approved. Your approved clinical record has not been changed.' } finally { saving.value=false } }
