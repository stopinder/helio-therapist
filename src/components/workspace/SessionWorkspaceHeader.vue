<template>
  <header class="bg-surface border-b border-border-muted px-inline-lg py-stack-md">
      ...
      <div class="flex items-center gap-inline-md">
        ...
        <div class="flex flex-col min-w-0">
          <div class="flex items-center gap-inline-sm flex-wrap">
            <h1 class="text-h2 font-semibold text-ink truncate">
              {{ session.clientName }}
            </h1>
    ...
    <p
        v-if="!isInPerson"
        class="mt-stack-sm text-caption text-ink-muted"
    >
      Zoom opens the video call in a separate tab. Keep Clinical Workspace open in Helio for session capture, notes and review.
    </p>

    <div
        v-if="meetingError"
        role="alert"
        class="mt-stack-sm rounded-control border border-state-danger/20 bg-state-danger/5 px-inline-md py-stack-xs text-body-sm text-state-danger"
    >
      {{ meetingError }}
    </div>
  </header>
