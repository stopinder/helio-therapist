      ...
      <div class="flex items-center gap-inline-md">
        ...
        <div class="flex flex-col min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-x-inline-md gap-y-0 text-caption text-ink-muted">
            <span>Next: {{ nextAppointmentLabel }}</span>
            <span>Therapist: {{ therapistLabel }}</span>
            <span v-if="client.archived_at">Archived {{ archiveDate }}</span>
          </div>
        </div>
      </div>
...
import { createOrResumeSession, listSessions } from '../../lib/sessions.js'
...
const supervisionPickerOpen = ref(false)
const supervisionReflections = ref([])
const supervisionSessions = ref([])
...
async function openClinicalWorkspace() {
  ...
  try {
    ...
    workspaceError.value = error?.code === 'CLIENT_ARCHIVED'
      ? 'Restore this client before opening Clinical Workspace.'
      : 'Couldn’t open Clinical Workspace. Please try again.'
  } finally {
...
async function openSupervisionPicker() {
  supervisionPickerOpen.value = true
  supervisionLoading.value = true
  supervisionError.value = ''
  supervisionSuccess.value = ''
  clearTimeout(supervisionSuccessTimer)
  try {
    ;[supervisionReflections.value, supervisionSessions.value] = await Promise.all([
      getPrivateReflectionsForClient({ clientId: props.client.id }),
      listSessions({ clientId: props.client.id })
    ])
  } catch (error) {
    console.error('Failed to load supervision choices:', error)
    supervisionError.value = 'Could not load private reflections for supervision.'
  } finally {
...
async function addSelectedToSupervision(ids) {
  ...
  try {
    ...
    if (updated.length) {
      supervisionSuccess.value = updated.length === 1 ? 'Saved to supervision.' : `${updated.length} reflections saved to supervision.`
      supervisionSuccessTimer = setTimeout(() => { supervisionSuccess.value = '' }, 8000)
    }
  } catch (error) {
    supervisionError.value = 'Could not add the selected reflection to supervision.'
  } finally {
...
async function requestArchiveChange() {
  ...
  const message = archiving
    ? `Archive ${props.client.display_name}? Their records will be retained and the client can be restored later.`
    : `Restore ${props.client.display_name} to the active caseload?`
  if (!window.confirm(message)) return
  archiveSaving.value = true
  archiveError.value = ''
  try { emit('client-updated', await setClientArchived({ clientId: props.client.id, archived: archiving })) }
