...
const supervisionPickerOpen = ref(false); const supervisionReflections = ref([]); const supervisionSessions = ref([]); const supervisionLoading = ref(false); const supervisionSaving = ref(false); const supervisionError = ref('');
async function openSupervisionPicker() { supervisionPickerOpen.value = true; supervisionLoading.value = true; supervisionError.value = ''; try { [supervisionReflections.value, supervisionSessions.value] = await Promise.all([getPrivateReflectionsForClient({ clientId: props.client.id }), listSessions({ clientId: props.client.id })]); } catch (error) { console.error('Failed to load supervision choices:', error); supervisionError.value = 'Could not load private reflections for supervision.'; } finally { supervisionLoading.value = false; } }
function closeSupervisionPicker() { if (!supervisionSaving.value) supervisionPickerOpen.value = false; }
async function addSelectedToSupervision(ids) { supervisionSaving.value = true; supervisionError.value = ''; try { const selected = supervisionReflections.value.filter(r => ids.includes(r.id) && !r.included_in_supervision); const updated = await Promise.all(selected.map(r => setReflectionSupervisionSelection({ reflectionId: r.id, included: true }))); const byId = new Map(updated.map(r => [r.id, r])); supervisionReflections.value = supervisionReflections.value.map(r => byId.get(r.id) || r); supervisionPickerOpen.value = false; } catch (error) { console.error('Failed to add supervision choices:', error); supervisionError.value = 'Could not add the selected reflection to supervision.'; } finally { supervisionSaving.value = false; } }
