      ...
      <div class="flex items-center gap-inline-md">
        ...
        <div class="flex flex-col min-w-0">
          ...
          <div class="flex flex-wrap items-center gap-x-inline-md gap-y-0 text-caption text-ink-muted">
            <span class="flex items-center gap-1">Next: {{ client.next_appointment }}</span>
            <span class="flex items-center gap-1">Therapist: {{ client.primary_therapist }}</span>
          </div>
        </div>
      </div>
...
const emit = defineEmits(['add-to-supervision']);
...
const openSession = async () => {
  try {
    ...
    router.push({ name: 'SessionWorkspace', params: { clientId: props.client.id, sessionId: session.id } });
  } catch (error) {
...
const joinMeeting = () => {
  videoProviderService.openMeeting({ videoProvider: 'zoom', meetingUrl: 'https://zoom.us/j/123456789' });
};

const videoLabel = computed(() => videoProviderService.getVideoActionLabel({
  videoProvider: 'zoom', meetingUrl: 'https://zoom.us/j/123456789', status: 'Scheduled'
}));

const clientInitials = computed(() => {
  if (!props.client.display_name) return '??';
  return props.client.display_name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
});
</script>
