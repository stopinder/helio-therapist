...
Inter is Helio's working typeface for controls, navigation, forms, notes, and dense interface content. Newsreader is the restrained editorial face for page titles, client names, and major section headings only. Both are loaded in `src/main.css` and configured in Tailwind.
...
| --- | --- | --- | --- |
| Display | `text-display` | 600 | Rare, high-level product moments only |
| H1 | `text-h1` | 600 | Workspace/page title |
...
- Use the serif only for orientation and hierarchy; never in navigation, buttons, forms, clinical notes, transcripts, or dense lists.
