...
test('Follow-ups UI components follow requirements', async () => {
  ...
  assert.ok(modal.includes("toggleRecording"))
  ...
  assert.match(panel, /<input\s+type="checkbox"/)
  assert.ok(panel.includes("Quick capture"))
})

test('Follow-ups migration follows schema requirements', async () => {
  const migration = await readFile(new URL('../supabase/migrations/20260814060601_add_client_follow_ups.sql', import.meta.url), 'utf8')
