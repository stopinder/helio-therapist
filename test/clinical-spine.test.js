...
test('working notes are durable, therapist-owned and bound to the matching session/client', async () => {
  const [library, migration] = await Promise.all([
  ...
  assert.match(library, /user_id: auth\.user\.id/)
  assert.match(library, /upsert\(payload, \{ onConflict: 'session_id' \}\)/)

  assert.match(migration, /alter table public\.session_working_notes enable row level security/)
  assert.match(migration, /user_id = \(select auth\.uid\(\)\)/)
  assert.match(migration, /session\.id = session_working_notes\.session_id/)
  assert.match(migration, /session\.client_id = session_working_notes\.client_id/)
})

test('clinical draft and completion use optimistic concurrency and one transactional completion boundary', async () => {
  const [sessions, migration] = await Promise.all([
    read('src/lib/sessions.js'),
    read('supabase/migrations/20260726113823_sprint_one_hardening.sql')
  ])

  assert.match(sessions, /rpc\('save_session_draft'/)
...
test('completed clinical record is rendered read-only and corrections are amendments', async () => {
  ...
  assert.match(clinicalSummary, /This approved record is read-only\. Corrections must be added through an amendment\./)
  assert.match(clinicalSummary, /<ApprovedClinicalRecordView/)
  assert.match(clinicalSummary, /Create Record Amendment/)
  assert.match(clinicalSummary, /completeSessionRecord/)
})
...
test('workspace remounts the clinical record when approval changes the session to completed', async () => {
  ...
  assert.match(workspace, /:key="clinicalRecordKey"/)
  assert.match(workspace, /@update:session="handleSessionUpdate"/)
  assert.match(workspace, /updatedSession\?\.status==='completed'/)
  assert.match(workspace, /clinicalRecordKey\.value\+=1/)
})
