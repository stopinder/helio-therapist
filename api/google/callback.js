...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!code || !state) return res.redirect(`${appUrl}/settings?google=error&message=Missing+OAuth+response`);

    const supabase = getSupabaseClient();
    const { data: oauthState, error: stateError } = await supabase.from('oauth_states').select('id, user_id, expires_at, completed_at').eq('id', state).eq('provider', 'google').maybeSingle();
    if (stateError || !oauthState || oauthState.completed_at || new Date(oauthState.expires_at).getTime() < Date.now()) {
    ...
    if(!clientId||!clientSecret||!redirectUri)return res.redirect(`${appUrl}/settings?google=error&message=Google+configuration+missing`);
    ...
    if(deleteError)console.warn('[Google Callback] Connected but failed to remove OAuth state:',deleteError);

    if(!hasGoogleCalendarReadScope(tokens.scope)){
      console.warn('[Google Callback] Google consent completed without Calendar read scope');
      return res.redirect(`${appUrl}/settings?google=error&message=Google+Calendar+permission+was+not+granted.+Reconnect+and+allow+Calendar+access.`);
    }
    return res.redirect(`${appUrl}/settings?google=success`);
  } catch(error){console.error('[Google Callback] Error:',error);return res.redirect(`${appUrl}/settings?google=error&message=Internal+server+error`)}
}
