...
import { CLIENT_SESSION_SUMMARY_LENSES, CLIENT_SESSION_SUMMARY_PROMPT_VERSION, buildClientSessionSummaryInput, clientSessionSummarySystemPrompt, validateClientSessionSummaryResponse } from '../_lib/ai-client-session-summary.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { clientId, lens='general', window='last_three' } = req.body || {};
    if (!clientId || typeof clientId !== 'string' || !/^[0-9a-f-]{36}$/i.test(clientId) || !CLIENT_SESSION_SUMMARY_LENSES.includes(lens) || !['current','last_three'].includes(window)) {
    ...
    const { data: captures, error: captureError } = await supabase
      .from('session_capture_drafts')
      .select('session_id,content,reviewed_at,sessions!inner(id,occurred_at,user_id,client_id)')
    ...
    const { data: careItems, error: careError } = await supabase
      .from('client_care_items')
      .select('kind,body,status,origin,provenance_session_id')
      .eq('client_id', clientId)
    ...
    const { completion } = await runTextAI({
      ...
      userId: user.id,
      promptVersion: CLIENT_SESSION_SUMMARY_PROMPT_VERSION,
      messages: [
        { role:'system', content:clientSessionSummarySystemPrompt(lens) },
        { role:'user', content:buildClientSessionSummaryInput({ lens, window, captures:selectedCaptures, careItems:careItems || [] }) }
      ],
      responseFormat:{ type:'json_object' },
      temperature:0.2,
      maxTokens:2200
    });
    const draft = validateClientSessionSummaryResponse(completion.choices?.[0]?.message?.content);
    if (!draft) return res.status(502).json({ success:false, error:{ code:'INVALID_AI_RESPONSE', message:'Client summary generation is temporarily unavailable.' } });
    return res.status(200).json({ success:true, data:{ draft, lens, window, sessionCount:selectedCaptures.length, promptVersion:CLIENT_SESSION_SUMMARY_PROMPT_VERSION } });
  } catch (error) {
    ...
    return res.status(status).json({ success:false, error:{ code:status===401?'UNAUTHORIZED':(error.code||'INTERNAL_SERVER_ERROR'), message:status===401?'Please sign in again.':'Client summary generation is temporarily unavailable. Nothing has been saved.' } });
  }
}
