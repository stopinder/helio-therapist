...
const PROMPT_VERSION = 'care-suggestions-v3';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!lensConfig) return res.status(400).json({ success:false, error:{ code:'INVALID_LENS', message:'The selected clinical lens is not available.' } });
    const ALLOWED_KINDS = lensConfig.allowedKinds;
