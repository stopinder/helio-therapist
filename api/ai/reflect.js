...
export default async function handler(req, res) {
  ...
  try {
    ...
    const hasReflectionId = bodyKeys.length === 1 && bodyKeys[0] === 'reflectionId';
    const hasReflectionText = bodyKeys.length === 1 && bodyKeys[0] === 'reflectionText';
    if (!hasReflectionId && !hasReflectionText) {
    ...
    if (hasReflectionId) {
      ...
      reflection = { body: typeof req.body.reflectionText === 'string' ? req.body.reflectionText.trim() : '' };
    }
